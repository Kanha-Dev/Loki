...
function createWindow() {
  ...
  // Invisibility + overlay behavior. Set SIXEYES_NO_PROTECT=1 to disable for debugging.
  win.setContentProtection(!process.env.SIXEYES_NO_PROTECT);            // excluded from screen capture (best-effort)
  ...
  win.webContents.on('render-process-gone', (_e, d) => console.log('[6Eyes] renderer gone', JSON.stringify(d)));
}
...
// getDisplayMedia loopback for system audio) so they run inside 6Eyes's own process
// and use 6Eyes's own Screen-Recording grant — no separate helper binary to authorize.
...
async function runFeature(mode, userText, images = null) {
  ...
  try {
    ...
    if (!imageDataUrls && def.needsScreen) {
      ...
      catch (e) { send('status', { message: 'Screen capture needs permission — grant Screen Recording to 6Eyes in System Settings.' }); }
    }
...
ipcMain.handle('meeting-note:generate', async (_e, previousNotes) => {
  ...
  const promptParts = [
    'You are 6Eyes, a meeting assistant that summarizes conversation into concise bullet points. Use the transcript below and the previous notes to generate updated meeting notes.',
...
app.whenReady().then(() => {
  ...
  // audio so the renderer can capture what's playing (Zoom/Meet) using 6Eyes's own grant.
